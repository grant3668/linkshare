...
## 1. 分支策略
- **`test`**：内容分支,**GitHub Pages 从 `test` 分支 `/(root)` 发布**。所有内容只放 `test`。
- **`main`**：保持**空**(只一行 README 或空提交)。是默认分支,别人打开仓库看不到内容。
- **`dev`**：**已弃用,不再维护**(全项目统一用 `test`)。
...
## 3. 历史策略：单提交 + 强推，不保留历史
- **`test` 永远只有 1 个 commit**;每次更新用 `git commit --amend --no-edit` 改写那一个 commit。
- **每次都用 `git push -f origin test` 强推**,丢弃历史(public 仓,不暴露过程)。
...
git push -f origin test                # 强推，无历史

# 回 tester 根仓 bump 子模块指针（linkshare 是 tester 的子模块，跟踪 test）
cd .. && git add linkshare && git commit -m "bump linkshare" && git push origin test
```

## 6. 与 tester 的关系
- linkshare 是 **tester 仓的子模块**,`.gitmodules` 里 `branch = test`。
- 固件源码在 `pixelhid/firmware/esp32_hid_bridge/`;发布用的合并 bin 由那边 build 出来后拷到这里的 `firmware/`。

## 7. GitHub Pages
- 地址:`https://grant3668.github.io/linkshare/`
- Source: Deploy from a branch → `test` /(root)。
